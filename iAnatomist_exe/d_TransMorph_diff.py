...
import d_finite_differences as fdt
import d_TransMorph as TM
import d_configs_TransMorph_diff as configs
...
def dice_coeff(input: Tensor, target: Tensor, reduce_batch_first: bool = False, epsilon=1e-6):
    ...
    if input.dim() == 3 or reduce_batch_first:
        ...
        for i in range(input.shape[0]):
            ...
            flag += dice_coeff(input[i, ...], target[i, ...])[1]
        if flag != 0:
            return dice / flag, 1
        else:
            return 0, 0
...
def multiclass_dice_coeff(input: Tensor, target: Tensor, reduce_batch_first: bool = False, epsilon=1e-6):
    ...
    for channel in range(input.shape[1]):
        ...
        flag += dice_coeff(input[:, channel, ...], target[:, channel, ...], reduce_batch_first, epsilon)[1]
...
class DiceLossOnLabel(nn.Module):
    ...
    def forward(self, source, target, weight_type='Simple', average=True):
        """
...
class TransMorphDiff(nn.Module):
    ...
    # def get_seg_loss(self, num_class,):
    #     seg = self.dice_loss(num_class)
    #     return seg

    def get_ncc_loss(self,):
        ncc = self.ncc_loss()
        return ncc
    ...
    # def dice_loss(self, num_class):
    #     """ segmentation loss"""
    #     dice = DiceLossOnLabel(num_class)
    #     return dice(torch.squeeze(
    #         F.one_hot(self.seg_target.to(torch.int64), num_class), dim=1).permute(0, 4, 1, 2, 3).float(),
    #         torch.squeeze(F.one_hot(self.seg_warped.to(torch.int64), num_class), dim=1).permute(0, 4, 1, 2, 3).float())
    #     # return dice_val(self.seg_target.to(torch.int64), self.seg_warped.to(torch.int64), num_class)
    #     # dice = DiceLoss().cuda()
    #     # return torch.mean(self.seg_target-self.seg_warped)

    def ncc_loss(self):
        ncc = VoxelMorphLNCC().cuda()
        return ncc(self.seg_target, self.seg_warped)
...
class TransMorphDiff_MutiTask(nn.Module):
    ...
    """
    def __init__(self, config):
        super(TransMorphDiff_MutiTask, self).__init__()
        img_sz = config.img_size
        image_sigma = config.image_sigma
        prior_lambda = config.prior_lambda
        prior_lambda_mean = config.prior_lambda_mean
        ...
        # 分割Decoder
        self.seg_up0 = TM.DecoderBlock(embed_dim * 8, embed_dim * 4, skip_channels=embed_dim * 4 if if_transskip else 0,
                                       use_batchnorm=False)
        self.seg_up1 = TM.DecoderBlock(embed_dim * 4, embed_dim * 2, skip_channels=embed_dim * 2 if if_transskip else 0,
                                       use_batchnorm=False)
        self.seg_up2 = TM.DecoderBlock(embed_dim * 2, embed_dim, skip_channels=embed_dim if if_transskip else 0,
                                       use_batchnorm=False)
        self.seg_up3 = TM.DecoderBlock(embed_dim, config.reg_head_chan,
                                       skip_channels=embed_dim // 2 if if_convskip else 0,
                                       use_batchnorm=False)
        self.seg_up4 = nn.Upsample(scale_factor=2, mode='trilinear', align_corners=False)
        self.seg_head = nn.Conv3d(config.reg_head_chan, 13, kernel_size=3, stride=1, padding=1, bias=True)
        ...
        # 边缘 Decoder
        self.edge_up0 = TM.DecoderBlock(embed_dim * 8, embed_dim * 4, skip_channels=embed_dim * 4 if if_transskip else 0,
                                       use_batchnorm=False)
        self.edge_up1 = TM.DecoderBlock(embed_dim * 4, embed_dim * 2, skip_channels=embed_dim * 2 if if_transskip else 0,
                                       use_batchnorm=False)
        self.edge_up2 = TM.DecoderBlock(embed_dim * 2, embed_dim, skip_channels=embed_dim if if_transskip else 0,
                                       use_batchnorm=False)
        self.edge_up3 = TM.DecoderBlock(embed_dim, config.reg_head_chan,
                                       skip_channels=embed_dim // 2 if if_convskip else 0,
                                       use_batchnorm=False)
        self.edge_up4 = nn.Upsample(scale_factor=2, mode='trilinear', align_corners=False)
        self.edge_head = nn.Conv3d(config.reg_head_chan, 1, kernel_size=3, stride=1, padding=1, bias=True)
    ...
    def forward(self, inputs, infer=False):
        ...
        # seg Decoder
        x1 = self.seg_up0(out[-1], f1)
        x1 = self.seg_up1(x1, f2)
        x1 = self.seg_up2(x1, f3)
        x1 = self.seg_up3(x1, f4)
        x1 = self.seg_up4(x1)
        seg = self.seg_head(x1)
        label = self.linear_stack(self.flatten(f1))

        # edge Decoder
        x1 = self.edge_up0(out[-1], f1)
        x1 = self.edge_up1(x1, f2)
        x1 = self.edge_up2(x1, f3)
        x1 = self.edge_up3(x1, f4)
        x1 = self.edge_up4(x1)
        edge = self.edge_head(x1)
        ...
        disp_field = self.scale_map(disp_field,np.array([1,1,1]))
        deform_field = disp_field + affine_map
        # deform_field = torch.tanh(deform_field)
        warped_source = self.bilinear_img(source, deform_field)
        ...
        return warped_source, deform_field, disp_field, seg, edge, label
...
class TransMorphDiff_Semi(nn.Module):
    """
    Probabilistic TransMorph Model
    :return: Warped image, Deformation field, Displacement field
    """
    def __init__(self, config):
        super(TransMorphDiff_Semi, self).__init__()
        img_sz = config.img_size
        image_sigma = config.image_sigma
        prior_lambda = config.prior_lambda
        prior_lambda_mean = config.prior_lambda_mean
        ...
        self.bilinear = STN_ND_BCXYZ(np.array([1.,1.,1.]),zero_boundary=True)
        self.bilinear_img = Bilinear(zero_boundary=True)
        self.bilinear_mask = Bilinear(zero_boundary=True)
        self.avg_pool = nn.AvgPool3d(3, stride=2, padding=1)
        if_convskip = config.if_convskip
        self.if_convskip = if_convskip
        if_transskip = config.if_transskip
    ...
    def forward(self, inputs, infer=False):
        ...
        deform_field = disp_field + affine_map
        warped_source = self.bilinear_img(source, deform_field)
        warped_seg = self.bilinear_mask(seg_mov, deform_field)
        ...
        return warped_source, deform_field, disp_field, seg, warped_seg

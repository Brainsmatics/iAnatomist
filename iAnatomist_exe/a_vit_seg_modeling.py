...
import a_vit_seg_configs as configs
from a_vit_seg_modeling_resnet_skip import ResNetV2, ResNetV2_3d
...
class Attention(nn.Module):
    def __init__(self, config, vis, reduce_ratio=4):
        ...
        self.sr = nn.Conv2d(self.all_head_size, self.all_head_size, reduce_ratio, reduce_ratio)
    ...
    def forward(self, hidden_states):
        ...
        if self.reduce_ratio > 1:
            p_x = hidden_states.clone().permute(0, 2, 1).reshape(query_layer.shape[0], self.all_head_size, 16, 32)
            sp_x = self.sr(p_x).reshape(query_layer.shape[0], self.all_head_size, -1).permute(0, 2, 1)
            hidden_states = self.norm(sp_x)
...
class Embeddings3d(nn.Module):
    ...
    def __init__(self, config, img_size, in_channels=3):
        ...
        if config.patches.get("grid") is not None:   # ResNet
            ...
            # n_patches = (img_size[0] // patch_size_real[0]) * (img_size[1] // patch_size_real[1])
            patch_size = (
                img_size[0] // 16 // grid_size[0], img_size[1] // 16 // grid_size[1], img_size[2] // 16 // grid_size[2])
...
class DecoderCup3d(nn.Module):
    ...
    def forward(self, hidden_states, features=None):
        B, n_patch, hidden = hidden_states.size()  # reshape from (B, n_patch, hidden) to (B, h, w, hidden)
        h, w, d = int(np.around(n_patch**(1/3))), int(np.around(n_patch**(1/3))), int(np.around(n_patch**(1/3)))
        x = hidden_states.permute(0, 2, 1)
